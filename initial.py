...
def line_Getter(img, gray, eye_avg):
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi/180, 50, minLineLength=30, maxLineGap=20) # detects all straight lines from the canny edges (returns array of lines)
    w = img.shape[1]
    final_Lines = []
    if len(lines) != 0: 
        for line in lines: 
...
def mask_Creator(lines,img):
    ...
    for line in lines:
        ...
        if((slope > 0.75 or slope < -0.75) and max(x1, x2) >= x_Max):
            x_Max = max(x1, x2)
        if((slope > 0.75 or slope < -0.75) and min(x1, x2) <= x_Min):
            x_Min = min(x1, x2)
    ...
    # print(x_Min)
    # print(y_Max)
    cv2.rectangle(mask_img, (x_Min, y_Max-30), (x_Max,h-50), (255,255,255), -1) # Drawing mask rectangle
...
if len(eye_y) > 0:
    eye_avg = (sum(eye_y)/len(eye_y))+ 20 # get average of eyes and look just below
else:
    print("NO EYES")
